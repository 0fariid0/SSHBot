...
if [[ ${EUID:-0} -ne 0 ]]; then
  echo -e "${RED}${BOLD}ERROR: run as root${RESET}"
  echo -e "${YELLOW}Example:${RESET} curl -fsSL https://raw.githubusercontent.com/0fariid0/SSHBot/main/install.sh | sudo bash"
  exit 1
fi
...
echo -e "${CYAN}${BOLD}"
echo "========================================="
echo " SSHBot Installer v3 (Embedded Pro Bot)"
echo " Keychain + Multi Upload + Python 3.12 fix"
echo "========================================="
echo -e "${RESET}"
...
if [[ -z "${BOT_TOKEN}" ]]; then
  if [[ -r /dev/tty ]]; then
    read -rp "$(echo -e ${YELLOW}'Enter Telegram Bot Token: '${RESET})" BOT_TOKEN </dev/tty
  else
    echo -e "${RED}ERROR: BOT_TOKEN is empty and no TTY is available.${RESET}"
    echo -e "${YELLOW}Use:${RESET} curl -fsSL https://raw.githubusercontent.com/0fariid0/SSHBot/main/install.sh | sudo BOT_TOKEN='YOUR_TOKEN' bash"
    exit 1
  fi
fi
...
echo -e "${BLUE}Stopping old service if exists...${RESET}"
...
echo -e "${BLUE}Installing OS dependencies...${RESET}"
...
mkdir -p "${INSTALL_DIR}" "${DATA_DIR}" "${KEYS_DIR}" "${UPLOAD_TMP_DIR}" "${LOG_DIR}"
chown -R "${BOT_USER}:${BOT_GROUP}" "${INSTALL_DIR}" "${LOG_DIR}"
chmod 700 "${KEYS_DIR}" || true
chmod 700 "${DATA_DIR}" || true

echo -e "${BLUE}Creating/updating virtualenv...${RESET}"
...
run_as_bot "${VENV_DIR}/bin/python" -m pip install -U --force-reinstall pip wheel "setuptools<81" >/dev/null 2>&1
run_as_bot "${VENV_DIR}/bin/python" -m pip install -U --force-reinstall \
  "setuptools<81" \
  "APScheduler==3.6.3" \
  "tzlocal<3" \
  "urllib3<2" \
  "python-telegram-bot==13.15" \
  certifi paramiko cryptography pyte >/dev/null 2>&1

echo -e "${BLUE}Testing Python imports...${RESET}"
run_as_bot "${VENV_DIR}/bin/python" - <<'PYTEST'
...
echo -e "${BLUE}Deploying embedded SSHBot Pro v3...${RESET}"
cat > "${BOT_FILE}" <<'PYBOT'
#!/usr/bin/env python3
import os
import sys
...
# Security: after install, set your Telegram numeric user IDs here if needed.
ALLOWED_USERS=
ALLOWED_CHATS=
PRIVATE_ONLY=1
...
echo -e "${BLUE}Creating systemd service...${RESET}"
cat > "${SERVICE_FILE}" <<EOF
[Unit]
Description=Telegram SSH Bot (SSHBot Pro v3)
...
if systemctl is-active --quiet sshbot; then
  echo -e "${GREEN}${BOLD}SSHBot Pro v3 installed and running.${RESET}"
  echo -e "${CYAN}Verify in Telegram with:${RESET} /version"
  echo -e "${CYAN}Server checks:${RESET}"
  echo "  grep BOT_VERSION /opt/sshbot/ssh-bot.py"
  echo "  journalctl -u sshbot -n 50 --no-pager"
else
  echo -e "${RED}${BOLD}SSHBot failed to start${RESET}"
  echo -e "${YELLOW}Logs:${RESET} journalctl -u sshbot -n 100 --no-pager"
  exit 1
fi
